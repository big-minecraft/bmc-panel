...
class PrometheusService {
    ...
    public async getPodCPUUsageForGraph(podName: string, namespace: string = ConfigManager.getConfig().namespace): Promise<TimeSeriesData[]> {
    ...
    public async getPodMemoryUsageForGraph(podName: string, namespace: string = ConfigManager.getConfig().namespace): Promise<TimeSeriesData[]> {
    ...
    public async getCurrentCPUUsage(podName: string, namespace: string = ConfigManager.getConfig().namespace): Promise<number> {
        const query = `sum(irate(container_cpu_usage_seconds_total{pod="${podName}", namespace="${namespace}", container!=""}[2m]))`;
        return await this.fetchInstantMetric(query);
    }

    public async getCurrentMemoryUsage(podName: string, namespace: string = ConfigManager.getConfig().namespace): Promise<number> {
        const query = `sum(container_memory_working_set_bytes{pod="${podName}", namespace="${namespace}", container!=""})`;
